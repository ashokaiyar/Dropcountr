 ...
 * - Native Hubitat "WaterSensor" capability mapping (wet/dry states)
 * - Properly structures events so water status binds to Dashboard tiles
 * - Only commits database state changes when attributes actually change
 * - Updated for Hubitat 2.5.1+ Map-based HTTP response headers
 *
...
metadata {
    definition(
        ...
        capability "WaterSensor" // Standardizes wet/dry states for Rule Machine, Dashboard, & Safety Monitor
        ...
        attribute "water",                  "enum", ["dry", "wet"] // Native attribute for WaterSensor capability
        attribute "leak",                   "enum", ["dry", "wet"] // Retained for backwards compatibility
...
void login() {
    ...
    try {
        httpPost(params) { resp ->
            // Fixed for Hubitat 2.5.1 platform Map parsing
            resp.headers.each { key, value ->
                if (key?.toLowerCase() == "set-cookie") parseCookieHeader(value)
            }
...
private void fetchMetrics() {
    ...
    apiGet(expandTemplate(ut, "day", todayDateRange())) { data ->
        ...
        String waterState = isLeaking ? "wet" : "dry"
        
        updateAttribute("leakDetected", isLeaking.toString())
        updateAttribute("leak", waterState)
        updateAttribute("water", waterState) // Core attribute required by Dashboard water templates
    }
...
private void updateAttribute(String name, def value, String unit = null) {
    ...
    if (device.currentValue(name) != value) {
        ...
        // Native Hubitat capability alignment: forces UI frameworks to register state types
        if (name == "water") {
            eventMap.descriptionText = "${device.displayName} is ${value}"
        }
...
private void schedulePoll() {
    ...
    switch (minutes) {
        ...
        case 30:
            ...
            break
        case 120:
            schedule("0 0 */2 ? * *", "refresh")
            break
        case 240:
            schedule("0 0 */4 ? * *", "refresh")
            break
        default:
